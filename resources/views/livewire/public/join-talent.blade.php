<div class="bg-surface-muted min-h-screen py-24">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        @if($isSubmitted)
            <x-card padding="p-16" class="text-center shadow-2xl mt-12">
                ...
                <p class="text-lg text-text-secondary mb-12 max-w-xl mx-auto">
                    Thanks for sharing your talent with us! We've sent a summary of your application to your inbox. 
                    Our team will take a close look at your portfolio and reach out if there's a spot for you in our network.
                </p>
        ...
        @else
            <x-heading align="center" subtitle="Join the Roster"
                title="Join the Hailerz Roster" emphasis="Hailerz Roster" class="mb-16" />
            <p class="mt--12 mb-16 text-center text-lg text-text-secondary">Become part of a premier network of performers,
                musicians, and creatives.</p>

            <!-- Progress Indicator -->
            <div class="mb-16 max-w-3xl mx-auto">
                <div class="relative">
                    <div class="overflow-hidden h-1.5 mb-6 text-xs flex rounded-full bg-subtle">
                        <div style="width: {{ ($currentStep / 5) * 100 }}%"
            ...
            <x-card padding="p-10 sm:p-16" class="shadow-2xl">
                <form wire:submit.prevent="submit" class="flex flex-col h-full">
                    ...
                    <div
                        class="flex-1 overflow-y-auto max-h-[60vh] pr-4 -mr-4 scrollbar-thin scrollbar-thumb-brand-primary/20 scrollbar-track-transparent">
                        ...
                        <div class="{{ $currentStep != 1 ? 'hidden' : 'block' }} space-y-10">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <x-input wire:model="artist_name" name="artist_name" label="Artist/Stage Name *"
                                ...
                                <x-input wire:model="profile_photo_url" name="profile_photo_url" label="Profile Photo URL *"
                                    placeholder="https://..." />
                            </div>
                        </div>
                        ...
                        <div class="{{ $currentStep != 3 ? 'hidden' : 'block' }} space-y-10">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <x-input wire:model="website_url" name="website_url" type="url" label="Website Link"
                                    placeholder="https://..." />
                                <x-input wire:model="instagram_handle" name="instagram_handle" type="url"
                                    label="Instagram Link" placeholder="https://instagram.com/..." />
                                <x-input wire:model="facebook_url" name="facebook_url" type="url" label="Facebook Page Link"
                                    placeholder="https://..." />
                                <x-input wire:model="youtube_channel" name="youtube_channel" type="url"
                                    label="YouTube Channel Link" placeholder="https://..." />
                                <x-input wire:model="tiktok_handle" name="tiktok_handle" type="url" label="TikTok Link"
                        ...
                        <div class="{{ $currentStep != 4 ? 'hidden' : 'block' }} space-y-10">
                            ...
                            <div class="grid grid-cols-1 gap-8">
                                <x-textarea wire:model="notable_venues" name="notable_venues"
                                    label="Notable Venues Performed" rows="3" />
                                <x-textarea wire:model="notable_clients" name="notable_clients"
                                    label="Notable Events/Clients" rows="3" />
                                <x-textarea wire:model="press_features" name="press_features" label="Press Features/Awards"
                                    rows="3" />
                            </div>
                            ...
                            <div class="pt-10 border-t border-subtle">
                                <div class="flex items-center justify-between mb-8">
                                    <h4 class="text-sm font-bold text-text-primary uppercase tracking-widest">Media Gallery
                                    </h4>
                                    <button type="button" wire:click="addGalleryItem"
                                        class="px-5 py-2.5 bg-brand-primary/10 text-brand-primary text-xs font-bold uppercase tracking-widest rounded-full flex items-center gap-2 hover:bg-brand-primary hover:text-text-inverse transition-all">
                                    ...
                                    @foreach($gallery as $index => $item)
                                        <div class="bg-surface-muted p-8 rounded-3xl border border-subtle relative group">
                                            <button type="button" wire:click="removeGalleryItem({{ $index }})"
                                                class="absolute top-6 right-6 text-text-muted hover:text-red-500 transition-colors">
                                                <x-lucide-x class="w-5 h-5" stroke-width="2" />
                                            </button>
                                            <div class="grid grid-cols-1 gap-6">
                                                <div>
                                                    <x-input wire:model="gallery.{{ $index }}.url" label="Media URL *"
                                                        placeholder="https://..." />
                                                    <p class="text-[10px] text-text-muted mt-2 italic">Links to YouTube,
                                                        SoundCloud, or portfolio images.</p>
                                                    @error('gallery.' . $index . '.url') <span
                                                    class="text-red-500 text-xs mt-2 block">{{ $message }}</span> @enderror
                                                </div>
                                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                    <div>
                                                        <x-input wire:model="gallery.{{ $index }}.title" label="Title" />
                                                    </div>
                                                    <div>
                                                        <x-input wire:model="gallery.{{ $index }}.description"
                                                            label="Description" />
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        <div class="{{ $currentStep != 5 ? 'hidden' : 'block' }} space-y-10">
                            ...
                            <div class="space-y-8">
                                <x-textarea wire:model="bio" name="bio" label="Artist Bio (Min 200 Characters) *"
                                    rows="6" />
                                <x-textarea wire:model="motivation" name="motivation"
                                    label="Why do you want to join Hailerz? *" rows="4" />
                    ...
                    <div class="flex items-center justify-start gap-4 mt-16 pt-10 border-t border-subtle">
                        ...
                        @else
                            <x-button variant="accent" size="lg" type="submit" wire:loading.attr="disabled"
                                wire:target="submit">
