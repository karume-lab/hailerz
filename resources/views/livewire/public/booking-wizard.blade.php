<div class="bg-canvas min-h-screen py-16">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @if($isComplete)
            <div class="bg-surface rounded-3xl p-12 text-center shadow-sm border border-border">
                ...
                <h2 class="text-3xl font-extrabold text-text-main mb-4">Inquiry Received</h2>
                ...
                <a href="/talent" wire:navigate class="inline-flex items-center justify-center px-8 py-4 border border-transparent text-base font-medium rounded-full text-white bg-primary hover:bg-primary-dark transition-colors">
                    Return to Directory
                </a>
            </div>
        @else
            <div class="mb-12 text-center">
                <h1 class="text-4xl font-extrabold text-text-main tracking-tight">Request a Quote</h1>
                <p class="mt-4 text-lg text-text-muted">Secure the perfect act for your next event.</p>
            </div>
            ...
            <div class="bg-surface shadow-[0_8px_30px_rgb(0,0,0,0.04)] sm:rounded-3xl p-8 sm:p-12 border border-border">
                <form wire:submit.prevent="submit">
                    
                    <!-- Step 1 -->
                    <div class="{{ $currentStep != 1 ? 'hidden' : 'block' }}">
                        <h3 class="text-2xl font-bold text-text-main mb-8">Event Details</h3>
                        
                        <div class="space-y-6">
                            <div>
                                <label for="talent_id" class="block text-sm font-semibold text-text-main mb-2">Requested Talent</label>
                                <select id="talent_id" wire:model="talent_id" class="block w-full px-4 py-3 border border-border rounded-xl focus:ring-primary focus:border-primary shadow-sm bg-canvas text-text-main form-select">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label for="event_date" class="block text-sm font-semibold text-text-main mb-2">Event Date</label>
                                    <input type="date" id="event_date" wire:model="event_date" class="block w-full px-4 py-3 border border-border rounded-xl focus:ring-primary focus:border-primary shadow-sm bg-canvas text-text-main">
                                    @error('event_date') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                                </div>
                                <div>
                                    <label for="event_location" class="block text-sm font-semibold text-text-main mb-2">Event City / Venue</label>
                                    <input type="text" id="event_location" wire:model="event_location" placeholder="e.g. Madison Square Garden, NYC" class="block w-full px-4 py-3 border border-border rounded-xl focus:ring-primary focus:border-primary shadow-sm bg-canvas text-text-main">
                            ...
                            <div>
                                ...
                                <textarea id="event_description" wire:model="event_description" rows="4" placeholder="Briefly describe the event, expected attendance, and what you expect from the talent..." class="block w-full px-4 py-3 border border-border rounded-xl focus:ring-primary focus:border-primary shadow-sm bg-canvas text-text-main"></textarea>
                    ...
                    <div class="{{ $currentStep != 2 ? 'hidden' : 'block' }}">
                        <h3 class="text-2xl font-bold text-text-main mb-8">Financial Commitment</h3>
                        
                        <div class="space-y-6">
                            <div>
                                <label for="proposed_budget" class="block text-sm font-semibold text-text-main mb-2">Proposed Talent Budget (USD)</label>
                    ...
                    <div class="{{ $currentStep != 3 ? 'hidden' : 'block' }}">
                        <h3 class="text-2xl font-bold text-text-main mb-8">Your Information</h3>
                        
                        <div class="space-y-6">
                            <div>
                                <label for="client_name" class="block text-sm font-semibold text-text-main mb-2">Full Name / Company</label>
                                <input type="text" id="client_name" wire:model="client_name" class="block w-full px-4 py-3 border border-border rounded-xl focus:ring-primary focus:border-primary shadow-sm bg-canvas text-text-main">
                                @error('client_name') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                            </div>

                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label for="client_email" class="block text-sm font-semibold text-text-main mb-2">Professional Email</label>
                                    <input type="email" id="client_email" wire:model="client_email" class="block w-full px-4 py-3 border border-border rounded-xl focus:ring-primary focus:border-primary shadow-sm bg-canvas text-text-main">
                                    @error('client_email') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                                </div>
                                <div>
                                    <label for="client_phone" class="block text-sm font-semibold text-text-main mb-2">Phone Number</label>
                                    <input type="tel" id="client_phone" wire:model="client_phone" class="block w-full px-4 py-3 border border-border rounded-xl focus:ring-primary focus:border-primary shadow-sm bg-canvas text-text-main">
