<div class="pb-24">
            ...
            <div class="flex flex-col lg:flex-row gap-16">
                ...
                <div class="w-full lg:w-1/2 flex flex-col">
                    <div class="mb-10 flex flex-wrap items-center gap-4">
                        <span class="px-4 py-1.5 bg-primary-100 text-primary-700 text-xs font-bold uppercase tracking-widest rounded-full">
                            {{ $talent->category?->name }}
                        </span>
                        @if($talent->is_featured)
                            <span class="px-4 py-1.5 bg-slate-900 text-white text-xs font-bold uppercase tracking-widest rounded-full">
                                Featured Artist
                            </span>
                        @endif
                    </div>
                    ...
                    <div class="flex items-center gap-6 mb-12 text-slate-500">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm font-semibold">Starts at ${{ number_format($talent->starting_price, 0) }}</span>
                        </div>
                    </div>
                    ...
                    <div class="mt-auto space-y-4">
                        <a href="/book?talent={{ $talent->id }}" class="w-full flex items-center justify-center px-8 py-5 bg-primary-600 text-white font-bold rounded-2xl hover:bg-primary-700 transition-all shadow-xl shadow-primary-200 text-lg">
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-24">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-16">
            <div>
                <h2 class="text-2xl font-black text-slate-950 mb-8 border-b border-slate-200 pb-4 flex items-center gap-3">
                ...
                <div class="bg-slate-50 rounded-3xl p-8 border border-slate-100">
                    ...
                    @else
                        <p class="text-slate-400 italic text-sm">No specific technical requirements listed. Please contact the agency for more details.</p>
                    @endif
                </div>
            </div>
            ...
            <div>
                ...
                <dl class="space-y-6">
                    <div class="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm">
                        ...
                        <dd class="text-slate-900 font-semibold">Base fee starts at ${{ number_format($talent->starting_price, 0) }}. Travel and accommodation expenses are calculated separately depending on the event location.</dd>
                    </div>
                    <div class="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm">
                        <dt class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Travel Policy</dt>
                        <dd class="text-slate-900 font-semibold">Available for travel worldwide. Travel within {{ $talent->location }} is included in the base fee.</dd>
                    </div>
                </dl>
            </div>
        </div>
